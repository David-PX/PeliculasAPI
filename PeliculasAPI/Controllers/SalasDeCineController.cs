...
namespace PeliculasAPI.Controllers
{
    ...
    public class SalasDeCineController : CustomBaseController
    {
        ...
        public async Task<ActionResult<List<SalaDeCineCercanoDTO>>> Cercanos ([FromQuery] SalaDeCineCercaoFiltroDTO filtro)
        {
            var ubicacionUsuario = geometryFactory.CreatePoint(new Coordinate(filtro.Longitud, filtro.Latitud));

            var salasDeCine = await context.SalasDeDines.OrderBy(x => x.Ubicacion.Distance(ubicacionUsuario))
