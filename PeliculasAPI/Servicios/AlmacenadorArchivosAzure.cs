    ...
    public class AlmacenadorArchivosAzure : IAlmacenadorArchivos
    {
        ...
        public async Task BorrarArchivo(string ruta, string contenedor)
        {
            if(ruta != null)
            {
                ...
                var contendorReferencia = Cliente.GetContainerReference(contenedor);

                var nombreBlob = Path.GetFileName(ruta);
                var blob = contendorReferencia.GetPageBlobReference(nombreBlob);
        ...
        public async Task<string> GuardarArchivo(byte[] contenido, string extension, string contenedor, string contentType)
        {
            ...
            await contendorReferencia.CreateIfNotExistsAsync();
            await contendorReferencia.SetPermissionsAsync(new BlobContainerPermissions
            {
            ...
            var blob = contendorReferencia.GetBlockBlobReference(NombreArchivo);
